...
export default function Tabs({
  ...
  const defaultPanelFound = panels.find(
    ([key]) => key === `panel.${defaultPanel}`
  );
  const firstPanelKey =
    defaultPanelFound !== undefined
      ? defaultPanelFound[0]
      : panels[0]?.[0] ?? "";
    ...
    <div className={styles.container}>
        ...
        >
          {tabs.map((tab) => (
            <button
              ...
              onClick={() =>
                updateCurr(tab.value, tabButtonRefs.current[tab.value])
              }
      ...
      {panels.map(([key, content]) => (
        <div
          ...
          role="tabpanel"
          aria-labelledby={`${tabId}-${getBaseKeyFromPanel(key)}`}
